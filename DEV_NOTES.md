...
- **draw.io export node spacing is not reliably overlap-free for larger topologies.** `export_to_drawio` uses `nx.spring_layout()` scaled by a fixed `CANVAS_SCALE` to place nodes, but `spring_layout` treats nodes as dimensionless points — it has no concept of the fixed `140x60` box size, so it can't guarantee any two boxes won't overlap, especially around densely-connected hub devices (e.g. a core switch with 17 neighbors). Tuning `spring_layout`'s `k`/`iterations` or scaling `CANVAS_SCALE` with node count are heuristic mitigations only, not a real fix. More reliable options: a Graphviz layout with explicit overlap removal (`sfdp`/`neato` with `overlap=false` — requires installing the Graphviz binary, a real added dependency), or relying on draw.io's own built-in auto-layout (Arrange → Layout → Organic/Tree) as the actual overlap-aware arrangement step after import, since draw.io's layout engine does know about box sizes. **Decision (2026-09-01): rely on draw.io's built-in auto-layout rather than building/improving a Python-side layout algorithm.** `export_to_drawio`'s current `spring_layout`-based positions are good enough as a rough starting point; no further code changes planned here. Run Arrange → Layout → Organic (or Tree) after opening the exported file if spacing looks off.
